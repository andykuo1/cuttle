...
export function find(component, selectors)
{
    return getRootElement(component).querySelector(selectors);
}

export function findAll(component, selectors)
{
    return getRootElement(component).querySelectorAll(selectors);
}
...
export function defineComponent(componentClass, name, defineOptions = undefined)
{
    ...
    let result = function attributeChangedCallback(attribute, prev, value)
    {
        ...
        if (ATTRIBUTE_CHANGED_LISTENERS_KEY in this)
        {
            ...
            if (listeners.has(attribute))
            {
                ...
                {
                    listener.call(undefined, ownedNext, ownedPrev, attribute);
                }
            }
